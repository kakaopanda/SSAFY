<template>
  <b-container class="bv-example-row mt-4 text-center">
<div class="container">

<b-carousel
      id="carousel-1"
      :interval="4000"
      controls
      indicators
      background="#ababab"
      img-width="1024"
      img-height="480"
      style="text-shadow: 1px 1px 2px #333;"
    >
      <b-carousel-slide img-src="https://picsum.photos/1024/480/?image=238"
        caption="부동산 거래 과정과 주의사항에 대해 알아보세요."
        text="Learn about the real estate transaction process and precautions."></b-carousel-slide>
      <b-carousel-slide img-src="https://picsum.photos/1024/480/?image=192"
        caption="부동산 정책 관련 최신 동향을 확인하세요."
        text="Check out the latest trends in real estate policy."
      >
      </b-carousel-slide>
      <b-carousel-slide img-src="https://picsum.photos/1024/480/?image=88"
        caption="궁금한 내용은 Q&A를 활용해주세요."
        text="If you have any questions, please use Q&A."></b-carousel-slide>
    </b-carousel>

    <br><br>
		<div class="col-sm-12">
			<div class="row">
				<div class="col-sm-6">
					<ul class="list-group list-group-flush mt-4">
						<li class="list-group-item"><h2>Tips</h2></li>
            <div align="left">
						<li class="list-group-item border-0"><img
							src="data:image/png;base64,"
							width="40" height="40" /> <a href="https://blog.naver.com/land_admin/221172176139" target='_blank' style="text-align:left;">&nbsp;1. 가용자금 확인 및 대출 계획</a></li>
						<li class="list-group-item border-0"><img
							src="https://e7.pngegg.com/pngimages/614/475/png-clipart-computer-icons-scalable-graphics-house-illustration-house-cdr-angle.png"
							width="40" height="30" /> <a href="https://blog.naver.com/land_admin/221172968369" target='_blank'>&nbsp;2. 집 종류 및 지역 선택</a></li>
						<li class="list-group-item border-0"><img
							src="data:image/jpeg;base64,"
							width="40" height="30" /> <a href="https://blog.naver.com/land_admin/221172975575" target='_blank'>&nbsp;3. 정보 수집 & 시세파악</a></li>
						<li class="list-group-item border-0"><img
							src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRKKLwH5KNeqEOraFqXjoqurLvoVT5RaYPx1Q&usqp=CAU"
							width="40" height="30" /> <a href="https://blog.naver.com/land_admin/221173052981" target='_blank'>&nbsp;4. 부동산 방문 & 집구경</a></li>
						<li class="list-group-item border-0"><img
							src="https://w1.pngwing.com/pngs/194/45/png-transparent-pencil-icon-drawing-icon-design-black-and-white-line-material-angle-hardware-accessory.png"
							width="40" height="30" /> <a href="https://blog.naver.com/land_admin/221176290264" target='_blank'>&nbsp;5. 계약 및 잔금 치르기</a></li>
						<li class="list-group-item border-0"><img
							src="https://cdn-icons-png.flaticon.com/512/4883/4883292.png"
							width="40" height="30" /> <a href="https://blog.naver.com/land_admin/221176300611" target='_blank'>&nbsp;6. 소유권 이전등기</a></li>
						<li class="list-group-item border-0"><img
							src="https://kr.seaicons.com/wp-content/uploads/2015/10/Engineering-icon.png"
							width="40" height="30" /> <a href="https://blog.naver.com/land_admin/221176306347" target='_blank'>&nbsp;7. 인테리어 공사</a></li>
							<li class="list-group-item border-0"><img
							src="https://img.icons8.com/ios-filled/344/inventory-flow.png"
							width="40" height="30" /> <a href="https://blog.naver.com/land_admin/221176998743" target='_blank'>&nbsp;8. 이사</a></li>
							<li class="list-group-item border-0"><img
							src="https://img.icons8.com/ios-filled/344/home.png"
							width="40" height="30" /> <a href="https://blog.naver.com/land_admin/221177010341" target='_blank'>&nbsp;9. 거주</a></li>
							<li class="list-group-item border-0"><img
							src="https://img.icons8.com/ios-glyphs/344/money--v1.png"
							width="40" height="30" /> <a href="https://blog.naver.com/land_admin/221177033918" target='_blank'>&nbsp;10. 매도하기</a></li>
            </div>
					</ul>
				</div>
				<div class="col-sm-6">
					<ul class="list-group list-group-flush mt-4">
						<li class="list-group-item bo"><h2>News</h2></li>
						<div id="news"></div>
            <div align="left" style="font-size:16px; margin-top:6px;">
                <li class="list-group-item border-0"> • 다음 주부터 분양가 12억 원 이하 아파트도 중도금 대출 허용</li>
                <li class="list-group-item border-0"> • 3년 전보다 싸게 팔렸다…쏟아진 급매물, 집값 어디까지</li>
                <li class="list-group-item border-0"> • 김포에 4만 6천 세대 미니 신도시…공급 지속 신호</li>
                <li class="list-group-item border-0"> • 김포에 미니 신도시 만든다…지하철 5호선 연장 예정</li>
                <li class="list-group-item border-0"> • 성일종 전세 사기 주요 원인은 임차인 정보 부족…보호 대책 논의</li>
                <li class="list-group-item border-0"> • 반포1단지 5천 세대로 재건축…서초역 주변엔 대규모 복합시설</li>
                <li class="list-group-item border-0"> • 서울 · 경기 4곳 빼고 규제 다 푼다는데…기대효과는?</li>
                <li class="list-group-item border-0"> • 건설사·금융사 휘청…정부, 미분양에 5조 대출 보증</li>
                <li class="list-group-item border-0"> • 서울·경기 4곳 빼고 규제지역 다 푼다…거래 살아날까</li>
                <li class="list-group-item border-0"> • [오뉴스 출연] 규제 지역 전격 해제 (정철진 경제평론가)</li>
            </div>
               
					</ul>
				</div>
			</div>
		</div>
	</div>
  <br>
  <the-footer></the-footer>
  </b-container>
</template>

<script>
import TheFooter from "@/components/TheFooter.vue";

export default {
  name: "AppMain",
    components: {
    TheFooter
  },
  methods : {
		
		
	}
};
</script>

<style scoped>
</style>
