...
//	public ResponseEntity<String> writeAnswer(@RequestBody @ApiParam(value = "게시글 정보.", required = true) QnaDto qnaDto) throws Exception {
//		logger.info("writeArticle - 호출");
//		if (qnaService.writeArticle(qnaDto)) {
//			return new ResponseEntity<String>(SUCCESS, HttpStatus.OK);
//		}
...
//	@GetMapping
//	public ResponseEntity<List<QnaDto>> listArticle(@ApiParam(value = "게시글을 얻기위한 부가정보.", required = true) BoardParameterDto boardParameterDto) throws Exception {
...
//	}
	...
	@ApiOperation(value = "게시판 글목록", notes = "모든 게시글의 정보를 반환한다.", response = List.class)
	@GetMapping
	public ResponseEntity<List<QnaDto>> listArticle() throws Exception {
		logger.info("listArticle - 호출");
